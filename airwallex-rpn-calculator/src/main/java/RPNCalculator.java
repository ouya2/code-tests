...
public class RPNCalculator
{
     ...
     * @throws Exception
     */
    private void evaluateRPNExpression(String inputExpression, boolean isUndoOperation) throws Exception
    {
    ...
    private void processOperator(String operatorString, boolean isUndoOperation) throws Exception
    {
        ...
        if (operator == null) {
            throw new IllegalArgumentException("Operator can not be found for operator: " + operatorString);
        }
        ...
        if (operator.getNumberOfParameters() > numbersStack.size()) {
            throw new IllegalStateException(String.format("operator %s (position: %d): insufficient parameters",
        ...
        // calculate
        Double result = operator.calculate(firstParameter, secondParameter);

        if (result != null) {
            numbersStack.push(result);
            if (!isUndoOperation) {
                instructionsStack.push(new Instruction(OperatorRegistry.getOperator(operatorString), firstParameter));
            }
        }
    }
